...
void Seleccion( string& in, int& cmt, string& cpal, float& tpal, int& nerr, string& morse, string& msg, int contf );
...
int main()           
{
	...
	if (!morse) {
        ...
        do
        {
            ...
            // En esta sección del código se cuenta la cantidad de líneas en el archivo
            // morse.txt. Podría modificarse fácilmente para añadir una validación adicional.
            getline( morse, aux, '\t' );
            getline( morse, aux, '\n' );
            contfile = contfile + 1;
            morse.peek();
        } while (!( morse.peek() == EOF ));
    }
    morse.close();
    ...
    do {
        ...
        InputString( menu0 , inp , 1 , 1 , cond0, "_1_2_3_4_5_", 1 );
        Seleccion( inp , cantmsg , cantpal , totpal , nerror , morsefinal, texto, contfile );
    }
    while ( !( inp == "5" ) );
    return 0;
};
...
void Seleccion( string& in, int& cmt, string& cpal, float& tpal, int& nerr, string& morse, string& msg, int contf )
{
    ...
    // Inicilaización de matriz traductora
        contm = 0;
    ...
    do
    {
        ...
        getline( file, matrix[contm][0], '\t' );
        getline( file, matrix[contm][1], '\n' );
        cout << matrix[contm][0] << " " << matrix[contm][1] << endl;
        cout << contm << " " << contf << endl;
        contm = contm + 1;
        file.peek();
    } while (!( file.peek() == EOF ));
    file.close();
    ...
    case 1:
        ...
        InputString( menu1 , in , 1 , 1 , cond1, "_1_", 1 );
        break;
    case 2:
